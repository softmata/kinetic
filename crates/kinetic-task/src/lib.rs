...
use kinetic_core::{Goal, KineticError, Pose};
...
use kinetic_trajectory::{
    trapezoidal_per_joint, TimedTrajectory, TrajectoryValidator, TrajectoryViolation,
};
...
/// Default velocity fallback when the URDF doesn't specify per-joint velocity limits.
const FALLBACK_MAX_VEL: f64 = 1.0;
/// Default acceleration fallback when the URDF doesn't specify acceleration.
const FALLBACK_MAX_ACCEL: f64 = 2.0;

/// Extract per-joint velocity and acceleration limits from the robot model.
///
/// Returns `(velocity_limits, acceleration_limits)` vectors of length `robot.dof`.
/// Falls back to `FALLBACK_MAX_VEL` / `FALLBACK_MAX_ACCEL` when limits are absent.
fn extract_joint_vel_accel(robot: &Robot) -> (Vec<f64>, Vec<f64>) {
...
fn detect_chain(robot: &Robot) -> std::result::Result<KinematicChain, TaskError> {
